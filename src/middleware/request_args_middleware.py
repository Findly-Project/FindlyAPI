...
class RequestArgsMiddleware:
    ...
    def checking_overlapping_arguments(self) -> bool:
        ...
        if exclusion_words and query:
            ...
            set_exclusion_words: set[str] = set(x.lower() for x in  exclusion_words.split("|"))
            if enable_name_filter in [None, NoneType, "on"] and all(
                [exclusion_words, query]
            ):
    ...
    def checking_enable_filter_by_price_arg(self) -> bool | CheckArgsEnum:
        ...
        if enable_filter_by_price_arg not in [None, NoneType, "off", "on"]:
            ...
            match enable_filter_by_price_arg:
                case "off":
                    enable_filter_by_price_arg: CheckArgsEnum = (
                        CheckArgsEnum.off_enable_filter_by_price_arg
                    )
                case "on":
                    enable_filter_by_price_arg: CheckArgsEnum = (
                        CheckArgsEnum.on_enable_filter_by_price_arg
                    )
                case _:
                    enable_filter_by_price_arg: CheckArgsEnum = (
                        CheckArgsEnum.on_enable_filter_by_price_arg
    ...
    def checking_enable_filter_by_name_arg(self) -> bool | CheckArgsEnum:
        ...
        if enable_filter_by_name_arg not in [None, NoneType, "off", "on"]:
            ...
            match enable_filter_by_name_arg:
                case "off":
                    enable_filter_by_name_arg: CheckArgsEnum = (
                        CheckArgsEnum.off_enable_filter_by_name_arg
                    )
                case "on":
                    enable_filter_by_name_arg: CheckArgsEnum = (
                        CheckArgsEnum.on_enable_filter_by_name_arg
                    )
                case _:
                    enable_filter_by_name_arg: CheckArgsEnum = (
                        CheckArgsEnum.on_enable_filter_by_name_arg
